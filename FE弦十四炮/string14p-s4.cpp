...
void AScript() {
    Init({
        AICE_SHROOM, AM_ICE_SHROOM, APUFF_SHROOM, ASUN_SHROOM, ASCAREDY_SHROOM,
        AFLOWER_POT, AWALL_NUT, ADOOM_SHROOM, ALILY_PAD
    });
    ...
    for (int w: {1, 4, 7, 11, 14, 17}) {
        ...
        P(w, 1236, 3, 8.1125);
        // 垫
        TempC(w, 600, APUFF_SHROOM, 2, 7, 700);
        TempC(w, 600, ASUN_SHROOM, 5, 7, 700);
        // 使用无偏移的植物垫，防止被冰车提前碾压
        TempC(w, 1016, ASCAREDY_SHROOM, 2, 7, 1050);
        TempC(w, 1016, AFLOWER_POT, 5, 7, 1050);
        // PPDD
    ...
    for (int w: {3, 6, 9, 10, 13, 16, 19}) {
        ...
        if (w != 9 && w != 19) {
            ...
            // 两次垫材
            TempC(w, a_len + 600, APUFF_SHROOM, 2, 7, 700);
            TempC(w, a_len + 600, ASUN_SHROOM, 5, 7, 700);
            TempC(w, a_len + 1016, ASCAREDY_SHROOM, 2, 7, 1050);
            TempC(w, a_len + 1016, AFLOWER_POT, 5, 7, 1050);
