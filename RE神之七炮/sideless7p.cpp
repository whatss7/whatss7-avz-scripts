...
void AScript() {
    WAInit({AICE_SHROOM, AM_ICE_SHROOM, ADOOM_SHROOM, ACOFFEE_BEAN, AFLOWER_POT, ACHERRY_BOMB, AKERNEL_PULT, ACOB_CANNON});
    WAAutoManageCob();
    WAFixCobs();
    ...
    for (int w: {20}) {
        ...
        RoofP(w, i2_len - 200, 6, 4, 9);
        A(w, 2050, 3, 8);
        RoofP(w, i2_len + a_len - 200, 1, 2, 9);
        RoofP(w, i2_len + a_len - 200, 6, 4, 9);
        RoofP(w, i2_len + a_len + a_len - 200, 1, 2, 9);
        RoofP(w, i2_len + a_len + a_len - 200, 6, 4, 9);
    }
}
