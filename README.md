...
- [whatss7-avz-scripts](#whatss7-avz-scripts)
  ...
  - [已完成的阵型](#已完成的阵型)
    ...
    - [FE 二十四炮](#fe-二十四炮)
      - [P6-2I](#p6-2i)
    - [ME 排曾五炮 C7u 原创 不稳定](#me-排曾五炮-c7u-原创-不稳定)
    - [RE L六炮 I6i 原创 不稳定](#re-l六炮-i6i-原创-不稳定)
    - [DE 水晶杯十炮 I4i](#de-水晶杯十炮-i4i)
  - [计划完成的阵型](#计划完成的阵型)
    - [PE 全金属四炮 ch4-N 手动 未完成](#pe-全金属四炮-ch4-n-手动-未完成)
    - [DE 斜八炮 ch4u 原创 未完成](#de-斜八炮-ch4u-原创-未完成)
    - [ME 核聚变六炮 P5 未完成](#me-核聚变六炮-p5-未完成)
    - [RE 方引号十二炮 ch4 未完成](#re-方引号十二炮-ch4-未完成)
...
[C6u: I-PPD | I-PPD | ND | PPA'D (1975, 1975, 750, 750)](./NE拐七炮/turn7p.cpp)
...
**\*: 本脚本挂机测试采用自然出怪，但针对自然出怪的刷新延迟问题没有进行解决。**
...
[ch6: IPP-PPDD | PPSSDD | IPP-PPDD | PPSSDD (11.5, 6, 11.5, 6)](./PE二十四炮/pe24p-ch6.cpp)

**本阵解需要两个存冰位，所以请使用[双冰存档](./PE二十四炮/双冰存档/game1_13.dat)。使用此阵解时，本阵为极限阵，不考虑阳光消耗。**

直接复制变奏二十二炮即可（

写出这个阵解也算是圆了我入坑时的目标，毕竟入坑的时候第一个能看懂的二十四炮阵解就这一个（

另外学到一点，如果本行没有巨人，PSD的P把本行僵尸炸完了，S可能延迟引信，因此D至少要delay110...

#### ch6（单存冰位）

[ch6: IPP-PPDD | PPSSDD | IPP-PPDD | PPSSDD (11.5, 6, 11.5, 6)](./PE二十四炮/pe24p-ch6-si.cpp)

2f还是掉1125，对比双存冰位如果把高坚换成坚果掉1250，优化了但不多（

不知道阳光能不能续航。好像无红关拖满能回个差不多有1000，就这样吧。

收尾只开一轮炮了，因此PSD改PDD收掉普僵。有时候3路普僵会漏，收巨人也没有顺手收掉，这时手动开下炮就行。

旗帜波是冰波的时候会有一些特殊情况。如果还打100冰1149波，小丑还没入场，当场就能把你的存冰炸掉。所以这里拖到230冰，并且热过渡炸实直接炸掉舞王。通过seml测试，旗帜波确实不太容易意外，双边炸实自然出怪也不会意外刷新。

### FE 二十四炮

#### P6-2I

[P6: PPSSDD | PPDD | PPSSDD | PPDD | PPI | PPI (6, 6, 6, 6, 6, 6)](./FE二十四炮/fe24p.cpp)

**本阵解无需存冰位，所以请使用[双花存档](./FE二十四炮/双花存档/game1_13.dat)。**

w10 炸蹦极的炮要复用到 w16 ice2 后的第一个PSD处，所以这里调整了 PSD 时机，PS 没能瞬杀的红眼也能被拦下来。上半场分一炮炸 1 路也是为了能拦下来，如果过早激活则 D 收不全撑杆。

w19 需要垫材，这次我写的那个最佳垫材时机给出的结果对应时机垫不到，没办法了用速度计算器拉了个折线图看看次优解是怎样的。说不定以后也会把这个做到工具里面...

### ME 排曾五炮 C7u 原创 不稳定

[C7u: I-PP | I-PP' | PA | N | PP (1527, 1527, 601, 746, 601)](./ME排曾五炮/ag5p.cpp)

**此阵为极限阵，不考虑阳光消耗。**

不是你三个星期就研究出来这么个破东西？（（（

这个阵的命题，是在天台环境下，通过 23 炮+曾的形状早激活，达成无拦截效果（一开始是想做一个 C4u 无拦截裸轨的阵，但是这个构型要求必须 PS 激活，实在复用不开；然后想出这个阵之后考虑的是C6u，但是对位C6u加速波出生的巨人到冰波投掷无论如何都会投到南瓜后，邻位又复用不开，所以进一步升到C7u）。

由于 23 炮的存在，必须在巨人到达 626 之前使其投掷，否则可能直接越过南瓜啃炮（虽说有铲种换炮问题不大）。因此，两个冰波波长之和不能超过 3252，传统的（指压碑五炮式的）C7u 复用直接不成立了，所以这里要加入铲种。而且 2 列炮极其难用，完全收不到撑杆，很难全收小丑橄榄。但是小丑是必须要收的（第二冰波5路小丑没有全收，所以 PA 波 A 必须放在 2 路；唯一一次放在 4 路那波是全收小丑的；但是即使不全收小丑也必须要全伤巨人），所以炸点很靠右，巨人会逃离爆炸范围。这时候排曾就有用了：会逃逸的巨人都是在冰波逃逸的，减速状态下巨人轻松收掉。

对于 2 列炮很难炸到下行撑杆的问题，轮流炸 24 路和 25 路，上波收不到的撑杆给下波收。

但是这里还是有问题。一是巨人：存在那种快，但不完全快的巨人，快到能逃离炮范围，但又慢到减速时间结束了还没进排曾攻击范围，于是原速冲过去一顿砸，这样的巨人都是核波出生的，所以 5 路垫一下，让这些巨人能尽量在第二冰波就被收掉（经过测试，5 路还是有砸率，不过我确实优化不动了）；二是跳跳：w17 的核放在了 4-8，这个位置无法全收 1 路跳跳，可能会偷家。不过这种跳跳刚跳过曾就会被减速，问题应该不大；三是收尾，收尾的逻辑毫无智能判断，可能会提前结束收尾导致冷却转不过来；四是南瓜，南瓜压力感觉挺大的，多跑几次选卡可能会被啃爆。

总而言之，2列炮真难用（

不过后来一想，用间C7u是不是能更好一些，对红眼压制更强，不过考虑复用的话冰波还得拉长，没仔细确认。

### RE L六炮 I6i 原创 不稳定

[上F: I6i: I3n-PPDD' | I-PPAd" | I-PP (1782, 1782, 1782); 下F: I6i: I-PPDD" | I-PP | I3n-PPAd' (1782, 1782, 1782)](./REL六炮/l6p.cpp)

**此阵为极限阵，不考虑阳光消耗。**

命题：六炮进行双边带容错的拦截。

5-5 炮在 I3n 操作下存在炮损，所以主要铲种这门炮。巨人不会直接把小鬼投到其他炮上，我觉得应该能算没有炮损。铲种尽量去铲 5-5 炮防止被啃穿，但是这里出现了一个情况就是冰车会碾压花盆，铲种和樱桃会抢花盆。问题也不大，晚点再补铲种的花盆就行。所有铲种留足了时间，黄油糊脸时巨人必然已经投出小鬼。有一种极端的黄油糊脸情况，就是糊本波小丑使得激活炮炸不到，然后冲到前面炸掉炮。这个疑似有点太极端了，不考虑吧。

为了方便收尾，收尾的下一波最好是 PPDD，这样可以把其中 cd 比较富裕的那一门炮挪过来用，打出一个 PSD。因此，上下半场节奏翻转过来，使得排波适合收尾。收尾方式如下（收尾懒得写全分布收尾了，就直接 Ensure 一下算了）：

- 上半场 w9 收尾是 PPI3n 波次，但是显然不可能再点冰 I3。所以前面核尽早放，晚点再放咖啡豆，使得此处核可以直接用，最后选择 J-PS-N-D 的排布，J 清冰车，PS 激活下半，N 拦截（用炮无法既全伤又拦截）；另外为了支撑 w10 的用炮需求，收尾实在拿不出运算量，只好 w1 存一个核在 w9 点掉。
  - 第二轮激活 PD/P。理论上，这波必须 12 路出僵尸，若 12 不出，则如果 4 路出且 5 路不出也行，其他排布无法拖满。

- 下半场 w19 收尾是 PPDA 波次。虽然没有核的大范围拦截，但是樱桃基本不需要保护就能放。最后采取 PD/JPSA 的排布，JPSA 瞬杀 5 路红眼，PSA 瞬杀 w18 的 4 路红眼，w19 的 4 路红眼投掷的小鬼不会越过南瓜头，用 5-4 曾收掉。这样可能会漏 w18 中路撑杆，所以 3-1 提前种一个南瓜，撑杆会跳过炮啃南瓜，这样收蹦极的时候会顺便收掉。
  - 第二轮激活 PSS；若 4 路不出则可以 PD-P ，PD 削成一血红然后 P 炸到剩一路。理论上这个收尾只要不是全出在 5 路就能收。

- w20 收尾就比较简单了，PND/NDD，D 尽可能晚从而多收普僵（还是不行的话可以 dc 快），234 路普僵红眼用一炮收掉，5 路普僵红眼用辣椒收掉，用 1 路红眼拖收尾。这里 N 要尽早用，给下一轮存核留时间。
  - 理论上这个收尾只要不是全出在 3 路和 5 路就能收。

另外，这里也需要处理极其难用的 23 炮。这次不仅会漏撑杆，还会漏扶梯跳跳。PPDD 波与 PPDA 波没什么问题，扶梯跳跳有拦截炮帮忙收，5-5 炮后面跟了个曾用来收撑杆；PPI3n 波则改炸 25 路，3 路的扶梯跳跳由核收，撑杆由下波的拦截炮收。但这某种程度上影响了收尾，w18 若是炸 24，则中路火力叠加，w19 不再有靠前的红眼，激活炮可以拦截中路小鬼；然而没有，所以 JPSA 都是炸 5 路，防止激活 3 路引发投掷。

全线砸率没有进行验证，目测会有砸率，但应该不会非常高（后来录视频的时候 w9 收尾锤过一次炮了）。

### DE 水晶杯十炮 I4i

[I4i: I-PPdd | I-PPdd (1564, 1564)](./DE水晶杯十炮/cc10p.cpp)

看起来是I4i练习，实际上是垫材练习。参考了几个视频都是手动下垫，而我做的阵通常都得挂机，所以研究了一下这个阵的垫材时机。垫材时机要避开冰冻，但不用避开投掷。一波两垫，取614和1365，其中614垫可以同时垫两波。

个人认为要想确保完全0砸率，必须要带四垫（当然三垫已经能解决绝大部分情况了），所以存两次核收尾从而压掉窝瓜，使得卡槽可以带上四垫。

还有就是智能垫材。如果不用智能垫材的话阳光暴降接近3000，使用后大概只降1500，倒也是挺多的。
...
相比于中开12炮，这里已经是首3尾0了，但是存冰还是回不上。目前的主要改进思路是双边不对称激活找机会存花盆，从而能使用寒冰射手维持减速线，但是这样又受到小丑的限制。
