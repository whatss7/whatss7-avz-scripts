...
首先，这个脚本只能打2f。循环的一些适配虽然有，但相当于没有。主要是因为，收尾波长是60s，执行两次循环后会让红眼白走10s。上路有额外的一炮问题不大，但下两路有可能逃离尾炸范围。

如果巨人情况最差，则每波需要用16次垫材（1路前场5次，后场5次，5路+6路各3次），而我们只有2张垫材。所以用了智能脚本，根据各条路的威胁（就是与理论最快的距离）判断垫哪里。但是智能脚本是这样的，锁个RND就能让脚本当场去世。（5次垫材是最优下垫时机下的结论，但因为要迁就下半场的时机，实际上上路需要6垫）

为了降低不稳定因素（主要是撑杆），小喷换成了大喷。分离炮通过调整时机和炸点保证全收跳跳、全伤当前波巨人。

就算大多数垫材都会分配给1路。1路经常会只受到3次灰烬伤害，就经常需要窝瓜解决残血巨人。但是4-7核波为了防小丑分离炮需要炸4路，3路冰车没人管了，会压掉一排大喷使得2路压力骤增，所以需要窝瓜来削减巨人血量并且干掉冰车。但4-7核波对1路的爆炸范围还很小，因此这里大概率1路也要个窝瓜，于是当场冲突去世。为了解决这个问题，4-7核波的上一波被安排为3-6核波（3-6波不容易出现需要窝瓜的情况），这一波前禁用窝瓜，留在这一波里提前用窝瓜压一次红眼，省出窝瓜留着解决4-7核波的冰车。同样，3-8核波也容易漏，3-7相对好一点。因此总的核坑循环就是`3-7` - `3-8` - `3-6` - `4-8`，高压波和低压波交替，使得窝瓜冷却可以缓和一点。
